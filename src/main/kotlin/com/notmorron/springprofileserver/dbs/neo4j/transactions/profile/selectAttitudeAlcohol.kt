...
fun NeoDB.selectAttitudeAlcohol(idUser: Long, attitudeAlcohol: String) = runCatching {
    transaction.use {
        it.executeWrite { tc ->

            val query = Query("""
                Match(p:Person{idUser: $idUser})
                optional Match(a:AttitudeAlcohol)
                where a is null
                Merge (aa:AttitudeAlcohol)
                with p, aa
                optional match (p)-[r]->(aa)
                delete r
                with p, aa
                Merge (p)-[:$attitudeAlcohol]->(aa)
            """.intern())
